...
Importing data into R effectively is crucial. For our tasks, we prefer the tibble format, a modern representation of data frames in R. The `load_expression()` function is designed to address a specific challenge: our CSV's first column doesn't have a header. This function smoothly loads the data, assigning "probeids" to the first column, ensuring compatibility with tibbles.

```{r load_expression}
library(tibble)
# Load the expression data
result_tib <- load_expression("/data/example_intensity_data_subset.csv")
head(result_tib) # Display the initial rows of the data
```
...
test_tib <- tibble(probeids=c('1_s_at', '2_s_at', '3_s_at', '4_s_at'),
                   ...
                   GSM6=rep(1.0, 4),
                   GSM7=rep(0.5, 4))
filtered_probes <- pull(filter_15(test_tib))
...
expression_data <- tibble(probeids = c("1_s_at", "2_s_at", "3_s_at"),
                          GSM1 = c(9.5, 7.6, 5.5),
                          GSM2 = c(9.7, 7.2, 2.9),
                          GSM3 = c(6.9, 4.3, 6.8))
gene_names <- tibble(affy_hg_u133_plus_2 = c("1_s_at", "2_s_at", "3_s_at"),
...
head(reduced_data) # Show the first few rows of the refined data

```

## Analysis of Gene Expressions

In our analysis, we aim to visualize the gene expressions categorized as 'good' and 'bad'. Before visualizing, it's crucial to ensure our data is structured correctly. For our visualization, we need the data in a wide format, so we'll start by converting our data to this format.
...
test_long <- tibble(probeids = c("probe1", "probe2"),
                    hgnc = c("geneA", "geneB"),
                    gene_set = c("good", "bad"),
                    sample1 = c(2.5, 3.0),
                    sample2 = c(2.7, 3.2))

wide_format_data <- convert_to_wide(test_long)
head(wide_format_data)
```
##Plotting the Boxplot
Now, with our data in wide format, we can visualize the distribution of gene expressions for the 'good' and 'bad' gene sets. We'll use ggplot2 to create a boxplot:
```{r}
library(tidyverse)  

# Melting the data into a longer format for ggplot boxplot
long_for_plotting <- wide_format_data %>%
  gather(key = "sample", value = "expression_value", -probeids, -hgnc, -gene_set)

library(ggplot2)

ggplot(long_for_plotting, aes(x = gene_set, y = expression_value)) + 
