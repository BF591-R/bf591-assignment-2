...
expression_data <- tibble(probe = c("1_s_at", "2_s_at", "3_s_at"),
                          GSM1 = c(9.5, 7.6, 5.5),
                          GSM2 = c(9.7, 7.2, 2.9),
                          GSM3 = c(6.9, 4.3, 6.8))
...
### Convert Data Format
First, we'll generate a sample tibble that is in wide format, your function should properly convert it to long format.

```{r}
# Test on an artificial tibble matching the structure of the real one
test_wide <- tibble(probe = c("probe1", "probe2"),
                    hgnc = c("geneA", "geneB"),
                    gene_set = c("good", "bad"),
                    GSM1 = c(2.5, 3.0),
                    GSM2 = c(2.7, 3.2))
...
ggplot(long_format_data, aes(x = gene_set, y = value)) + 
